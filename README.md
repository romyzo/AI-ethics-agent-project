...
<img width="820" height="552" alt="image" src="https://github.com/user-attachments/assets/90022ca4-5363-46aa-ac9b-877808e83b60" />
...
이번 프로젝트를 통해 LangGraph 기반 멀티에이전트 구조를 실제로 설계, 구현하며 다음과 같은 주요 인사이트를 얻을 수 있었습니다.

1️⃣ **아키텍처 완성도와 흐름 제어의 중요성**  
   - Supervisor Agent 중심의 단계적 실행 구조를 설계했지만, 노드 간 데이터 전달(State 구조)와 에이전트 간 의존 관계를 더 명확히 정의할 필요가 있음
   - 특히 “보고서 작성 → PDF 변환” 단계에서 출력 포맷을 일관되게 유지하기 위한 로직 보완 필요

2️⃣ **최종 보고서(Report Composer)의 품질 향상 필요성**  
   - 현재 생성되는 보고서는 구조적이지만 문단 간 자연스러운 흐름과 요약/강조 문구 정제 등에서 개선 필요
   - 향후 Markdown 템플릿을 세분화하거나, LLM 결과를 후처리(Post-processing)하는 알고리즘을 추가 필요

3️⃣ **입력 데이터 처리 자동화의 필요성**  
   - 현재는 사용자가 단일 파일(PDF, txt 등)을 직접 업로드해야 하지만, 다양한 문서를 폴더 단위로 읽어 서비스별로 자동 분류하는 기능 필요
   -> 이를 통해 여러 AI 서비스의 윤리 리스크를 한 번에 비교와 진단까지 수 있는 구조로 확장할 수 있음

4️⃣ **UX 개선의 가능성** 
   - 추후 간단한 **웹 UI 형태(Drag & Drop 업로드 + 결과 요약 보기)** 로 구현하면 접근성과 활용성이 크게 향상될 것입니다.

---

## 🚀 Future Development Direction

1️⃣ **자동 문서 인식 및 다중 서비스 분석 지원**  
   - `data/services/` 폴더 내 다수의 서비스 관련 문서를 자동 인식하여 각 서비스별로 윤리 리스크 평가를 병렬로 수행할 수 있도록 구조 확장

2️⃣ **UI 기반 상호작용 환경 구축**  
   - 웹 대시보드 형태의 인터페이스 제공 
   - 사용자가 파일을 업로드하면, 즉시 결과 요약과 리스크 차트, 개선 권고안 목록을 시각적으로 확인할 수 있도록 함

3️⃣ **리스크 평가 로직의 정량화 및 시각화 강화**  
   - EU AI Act의 리스크 구분 기준(Prohibited / High / Limited / Minimal)을 기반으로 점수화(0~100)한 후, 그래프 형태로 시각화
   -> 단순 텍스트 보고서가 아닌 **데이터 기반 평가 대시보드**로 진화시키는 방향
