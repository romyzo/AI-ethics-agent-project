...
ai_ethics_agent/
├── agents/
│   ├── service_profiler.py / .ipynb
│   ├── evidence_collector.py / .ipynb
│   ├── risk_assessor.py / .ipynb
│   ├── mitigation_recommender.ipynb
│   └── report_composer.py / .ipynb
├── data/
│   ├── reference/ (EU_AI_Act.pdf, OECD_Privacy_2024.pdf, UNESCO_Ethics_2021.pdf)
│   ├── crawled/ (웹 검색 데이터)
│   └── embeddings/ (Chroma 벡터 DB 저장소)
├── output/reports/ (Markdown 및 PDF 보고서)
├── utils/ (State 및 공통 유틸 관리)
│   ├── state_manager.py
│   ├── agent_state.json
│   └── __init__.py
├── main.py (Supervisor 파이프라인 실행)
├── state.ipynb (LangGraph 상태 정의)
├── .env (API Key 환경 변수)
└── README.md
